...
export default function App() {
  const [data, setData]           = useState({ users: DEFAULT_USERS, requests:{}, tasks:[], workTime:{}, machines:[] });
  ...
  // ── Firebase sync (REST API + polling) ──────────────────────────
  useEffect(() => {
    let active = true;
    let interval = null;
    ...
    (async () => {
      try {
        await fetch(FIREBASE_URL, {
          ...
          body: JSON.stringify({ requests: data.requests, tasks: data.tasks, workTime: data.workTime, machines: data.machines })
        });
  ...
  if (!currentUser) return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1e3a5f,#2d6a9f)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#fff", borderRadius:16, padding:32, width:"100%", maxWidth:340, boxShadow:"0 8px 32px rgba(0,0,0,0.2)" }}>
        ...
        <p style={{ margin:"0 0 4px", color:"#aaa", textAlign:"center", fontSize:12 }}>Колдаева Юлия Юрьевна</p>
        ...
        <button onClick={() => {
          ...
          if (u) { setCurrentUser(u); setLoginErr(""); setTab(u.role === "manager" ? "overview" : "plan"); }
  ...
  const MachinesPanel = () => (
    <div>
      ...
      {data.machines.length === 0 && <div style={{ color:"#aaa", textAlign:"center", padding:20 }}>Список техники пуст</div>}
      {data.machines.map(m => (
        <div key={m.id} style={{ ...S.card, display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderLeft:`3px solid ${m.repair?"#e74c3c":"#27ae60"}` }}>
          ...
          <button onClick={() => upd(d => {
            ...
            if (!mm.repair) {
              mm.repair = true;
              mm.repairSince = todayStr();
            } else {
              mm.repair = false;
              if (!mm.repairLog) mm.repairLog = [];
              mm.repairLog.push({ from: mm.repairSince || todayStr(), to: todayStr() });
              mm.repairSince = null;
            }
          })}
          ...
          <button onClick={() => upd(d => { d.machines = d.machines.filter(x => x.id !== m.id); })}
      ...
      <div>
        ...
        {planReqs.length === 0 && (
          <div style={{ color:"#888", textAlign:"center", padding:24 }}>
            {currentUser.role==="driver"?"🚗":"📋"} На {fmtDate(selDate)} заявок нет
          </div>
        )}
        {planReqs.map(r => {
            ...
            <div key={r.id} style={{ ...S.card, padding:"10px 14px", borderLeft:`3px solid ${r._mine||currentUser.role==="manager"?"#2d6a9f":"#8e44ad"}` }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                <button onClick={() => {
                  ...
                  upd(d => {
                    ...
                    if (r._type === "task") {
                      ...
                      if (t) { t.done = !t.done; t.doneBy = t.done ? stamp : null; }
                    } else {
                      const uid = r._creatorId ?? currentUser.id;
                      const req = d.requests[uid]?.[selDate]?.find(x => x.id === r.id);
                      if (req) { req.done = !req.done; req.doneBy = req.done ? stamp : null; }
                    }
                  });
                ...
                {(r._mine||currentUser.role==="manager") && !r._type && (
                  <button onClick={() => upd(d => {
                    const uid = r._creatorId ?? currentUser.id;
                    d.requests[uid][selDate] = d.requests[uid][selDate].filter(x => x.id !== r.id);
                  })} style={{ background:"#fde8e8", border:"none", borderRadius:6, padding:"3px 8px", cursor:"pointer", fontSize:11, color:"#c0392b" }}>✕</button>
                )}
              </div>
        ...
        <div style={{ padding:14, maxWidth:700, margin:"0 auto" }}>
          ...
          {manTab === "overview" && (() => {
            const weekDays = Array.from({length:7}, (_,i) => {
              const d = new Date(manDate); d.setDate(d.getDate()+i);
              ...
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10, flexWrap:"wrap" }}>
                  ...
                  <div style={{ display:"flex", background:"#fff", borderRadius:8, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.07)" }}>
                    <button onClick={()=>setOvMode("day")} style={{ padding:"7px 14px", border:"none", cursor:"pointer", fontSize:13, fontWeight:ovMode==="day"?700:400, background:ovMode==="day"?"#2d6a9f":"#fff", color:ovMode==="day"?"#fff":"#666" }}>День</button>
                    <button onClick={()=>setOvMode("week")} style={{ padding:"7px 14px", border:"none", cursor:"pointer", fontSize:13, fontWeight:ovMode==="week"?700:400, background:ovMode==="week"?"#2d6a9f":"#fff", color:ovMode==="week"?"#fff":"#666" }}>Неделя</button>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    {sorted.map(emp => {
                      const weekData = weekDays.map(date => ({
                        date,
                        items: getReqsForDateByEmployee(emp.id, date),
                        wt: data.workTime[emp.id]?.[date] || {}
                      }));
                      ...
                      return (
                        <div key={emp.id} style={{ ...S.card, borderLeft:`4px solid ${totalItems>0?"#2d6a9f":"#ddd"}` }}>
                          <div style={{ fontWeight:700, color:"#1e3a5f", fontSize:14, marginBottom:8 }}>
                            {emp.role==="driver"?"🚗":"👤"} {emp.name}
                          </div>
                          <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
                            <span style={S.tag({ background:"#e8f7ee", color:"#27ae60" })}>📋 {totalDone}/{totalItems} заявок</span>
                            <span style={S.tag({ background:"#e8f4fd", color:"#2d6a9f" })}>⏱ {totalHours.toFixed(1)} ч</span>
                            {emp.role==="driver" && totalTrips>0 && <span style={S.tag({ background:"#fef3e2", color:"#e67e22" })}>🚗 {totalTrips} рейсов</span>}
                          </div>
                            ...
                            {weekData.map(({date, items, wt}) => {
                              ...
                              return (
                                <div key={date} style={{ flex:"1 0 auto", minWidth:42, textAlign:"center", background:busy?"#e8f4fd":"#f7f7f7", borderRadius:8, padding:"6px 4px" }}>
                                  ...
                                  <div style={{ fontSize:13, fontWeight:700, color:busy?"#2d6a9f":"#ccc", marginTop:2 }}>
                                    {busy ? `${items.filter(r=>r.done).length}/${items.length}` : "—"}
                                  </div>
                ...
                {ovMode==="day" && (() => {
                  const dayTotals = employees.reduce((acc, emp) => {
                    const items = getReqsForDateByEmployee(emp.id, manDate);
                    const wt = data.workTime[emp.id]?.[manDate] || {};
                    acc.total += items.length;
                    acc.done  += items.filter(r=>r.done).length;
                    acc.hours += Number(calcHours(wt.from, wt.to)||0);
                    acc.trips += Number(wt.trips||0);
                    return acc;
                  }, { total:0, done:0, hours:0, trips:0 });
                  return (
                  ...
                  return (
                    <div key={emp.id} style={{ ...S.card, borderLeft:`4px solid ${items.length>0?"#2d6a9f":"#ddd"}` }}>
                      ...
                      <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:items.length?8:0 }}>
                        ...
                        {emp.role==="driver" && wt.trips && <span style={S.tag({ background:"#fef3e2", color:"#e67e22" })}>🚗 {wt.trips} рейсов</span>}
                        {emp.role==="driver" && wt.route && <span style={S.tag({ background:"#e8f7ee", color:"#27ae60" })}>📍 {wt.route}</span>}
                        {emp.role==="driver" && wt.org && <span style={S.tag({ background:"#f0e8fd", color:"#8e44ad" })}>🏢 {wt.org}</span>}
                      </div>
                      {items.map(r => {
                        const m = data.machines.find(x => x.id === Number(r.machineId));
                        return (
                          <div key={r.id} style={{ padding:"5px 0", borderTop:"1px solid #f5f5f5", fontSize:13 }}>
                            ...
                            {r.done && r.doneBy && <div style={{ marginLeft:24, marginTop:2, fontSize:11, color:"#27ae60" }}>✓ Отметил: {r.doneBy}</div>}
                          </div>
                        );
                      })}
          ...
          {manTab === "machines_overview" && (() => {
            ...
            return (
              <div>
                ...
                {sorted.length===0 && <div style={{ color:"#aaa", textAlign:"center", padding:30 }}>Список техники пуст</div>}
                {sorted.map(m => {
                  const busy = isBusy(m);
                  const usedReqs  = getAllReqsForDate(manDate).filter(r => r.machineId===m.id);
                  const usedTasks = data.tasks.filter(t => t.machineId===m.id && t.date===manDate);
                  return (
                    <div key={m.id} style={{ ...S.card, borderLeft:`4px solid ${m.repair?"#f39c12":busy?"#27ae60":"#e74c3c"}` }}>
                      ...
                      {usedReqs.map(r => (
                        <div key={r.id} style={{ display:"flex", alignItems:"center", gap:6, padding:"5px 0", borderTop:"1px solid #f5f5f5", fontSize:13 }}>
                          <span>{r.done?"✅":"⏳"}</span>
                          <span style={{ flex:1 }}>{r.title}</span>
                          <span style={S.tag({ background:"#e8f4fd", color:"#2d6a9f" })}>📋 {r.userName.split(" ")[0]}</span>
                        </div>
                      ))}
                      {usedTasks.map(t => {
                        const emp = data.users.find(u => u.id===t.assignedTo);
                        return (
                          <div key={t.id} style={{ display:"flex", alignItems:"center", gap:6, padding:"5px 0", borderTop:"1px solid #f5f5f5", fontSize:13 }}>
                            <span>{t.done?"✅":"⏳"}</span>
                            <span style={{ flex:1 }}>{t.title}</span>
                            <span style={S.tag({ background:"#f0e8fd", color:"#8e44ad" })}>✅ {emp?.name.split(" ")[0]||"?"}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div>
              <div style={S.card}>
                ...
                <input value={newTask.title} onChange={e => setNewTask(p => ({ ...p, title:e.target.value }))}
                  placeholder="Описание задачи..." style={{ ...S.input, width:"100%", marginBottom:8 }} />
                ...
                <select value={newTask.machineId} onChange={e => setNewTask(p => ({ ...p, machineId:e.target.value }))}
                  style={{ ...S.select, width:"100%", marginBottom:10 }}>
                  <option value="">🚜 Без техники</option>
              ...
              {data.tasks.map(task => {
                const emp = data.users.find(u => u.id===task.assignedTo);
                const tm  = data.machines.find(m => m.id===task.machineId);
                return (
                  <div key={task.id} style={{ ...S.card, display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <button onClick={() => upd(d => { const t=d.tasks.find(x=>x.id===task.id); if(t) t.done=!t.done; })}
                      style={{ ...S.btn(task.done?"#eee":"#e8f4fd"), color:"#555" }}>{task.done?"Открыть":"✓"}</button>
                    <button onClick={() => upd(d => { d.tasks=d.tasks.filter(x=>x.id!==task.id); })}
                      style={{ ...S.btn("#fde8e8"), color:"#c0392b" }}>✕</button>
                  </div>
                );
              })}
            </div>
          ...
          {manTab==="report" && currentUser.role === "manager" && (() => {
            const monthStr = String(repMonth).padStart(2,"0");
            const daysInMonth = new Date(repYear, repMonth, 0).getDate();
            const allDays = Array.from({length: daysInMonth}, (_,i) => `${repYear}-${monthStr}-${String(i+1).padStart(2,"0")}`);
            const drivers = data.users.filter(u => u.role==="driver");

            const getItemsForDay = (date) => {
              const result = [];
              data.users.forEach(u => {
                (data.requests[u.id]?.[date] || []).forEach(r => result.push({ ...r, _creatorId: u.id }));
              });
              data.tasks.filter(t => t.date===date).forEach(t => result.push({ ...t, _type:"task" }));
              return result;
            };

            const buildCopyText = () => {
              const monthName = new Date(repYear, repMonth-1).toLocaleString("ru", {month:"long", year:"numeric"});
              let txt = `ОТЧЁТ ЗА ${monthName.toUpperCase()}\n${"=".repeat(40)}\n\n`;
              if (repSub==="machines") {
                data.machines.forEach(m => {
                  txt += `🚜 ${m.name}\n${"-".repeat(30)}\n`;
                  let totalTrips=0, busyDays=0;
                  allDays.forEach(date => {
                    const items = getItemsForDay(date).filter(r => Number(r.machineId)===m.id && r.done);
                    if (items.length===0) return;
                    busyDays++;
                    items.forEach(r => {
                      const emp = data.users.find(u=>u.id===Number(r.assignedTo||r._creatorId));
                      const wt  = data.workTime[emp?.id]?.[date]||{};
                      totalTrips += Number(wt.trips||0);
                      txt += `  ${fmtDate(date)}: ${emp?.name||"?"} | ${wt.org||"—"} | ${wt.route||"—"} | рейсов: ${wt.trips||0} | ${wt.from||"—"}–${wt.to||"—"}\n`;
                    });
                  });
                  txt += `  Итого: ${busyDays} дней в работе, ${daysInMonth-busyDays} дней простоя, ${totalTrips} рейсов\n\n`;
                });
              } else {
                drivers.forEach(drv => {
                  txt += `🚗 ${drv.name}\n${"-".repeat(30)}\n`;
                  let totalTrips=0, totalHours=0, workDays=0;
                  allDays.forEach(date => {
                    const wt = data.workTime[drv.id]?.[date]||{};
                    const items = getReqsForDateByEmployee(drv.id, date).filter(r => r.done);
                    if (items.length===0) return;
                    workDays++;
                    const h = Number(calcHours(wt.from,wt.to)||0);
                    totalHours += h;
                    totalTrips += Number(wt.trips||0);
                    const machines = [...new Set(items.filter(r=>r.machineId).map(r=>data.machines.find(m=>m.id===Number(r.machineId))?.name).filter(Boolean))];
                    txt += `  ${fmtDate(date)}: ${wt.from||"—"}–${wt.to||"—"} (${h}ч) | ${wt.org||"—"} | ${wt.route||"—"} | рейсов: ${wt.trips||0}${machines.length?` | 🚜 ${machines.join(", ")}`:""}\n`;
                  });
                  txt += `  Итого: ${workDays} раб.дней, ${totalHours.toFixed(1)} ч, ${totalTrips} рейсов\n\n`;
                });
              }
              return txt;
            };
            ...
            return (
              <div>
                <div style={{ ...S.card, display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
                  <div>
                    <label style={{ fontSize:12, color:"#555", display:"block", marginBottom:3 }}>Месяц</label>
                    <select value={repMonth} onChange={e=>setRepMonth(Number(e.target.value))} style={S.select}>
                      {["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"].map((n,i)=>(
                        <option key={i+1} value={i+1}>{n}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize:12, color:"#555", display:"block", marginBottom:3 }}>Год</label>
                    <select value={repYear} onChange={e=>setRepYear(Number(e.target.value))} style={S.select}>
                      {[2024,2025,2026,2027].map(y=><option key={y} value={y}>{y}</option>)}
                    </select>
                  </div>
                  <button onClick={() => {
                    try { navigator.clipboard.writeText(buildCopyText()); } catch {}
                  }} style={{ ...S.btn("#27ae60"), marginTop:18 }}>📋 Копировать отчёт</button>
                </div>
                ...
                <div style={{ display:"flex", background:"#fff", borderRadius:10, marginBottom:12, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.07)" }}>
                  {[{id:"machines",label:"🚜 По технике"},{id:"drivers",label:"🚗 По водителям"}].map(s=>(
                    <button key={s.id} onClick={()=>setRepSub(s.id)}
                      style={{ flex:1, padding:"10px 4px", border:"none", cursor:"pointer", fontSize:13, fontWeight:repSub===s.id?700:400,
                        background:repSub===s.id?"#2d6a9f":"#fff", color:repSub===s.id?"#fff":"#666" }}>
                      {s.label}
                    </button>
                  ))}
                </div>
                ...
                {repSub==="machines" && (
                  <div>
                    {data.machines.length===0 && <div style={{color:"#aaa",textAlign:"center",padding:20}}>Нет техники</div>}
                    {[...data.machines].sort((a, b) => {
                      const daysA = allDays.filter(date => getItemsForDay(date).some(r => Number(r.machineId) === a.id && r.done)).length;
                      const daysB = allDays.filter(date => getItemsForDay(date).some(r => Number(r.machineId) === b.id && r.done)).length;
                      return daysB - daysA;
                    }).map(m => {
                      const repairDays = allDays.filter(date => {
                        const periods = [...(m.repairLog || [])];
                        if (m.repair && m.repairSince) periods.push({ from: m.repairSince, to: "9999-12-31" });
                        return periods.some(p => date >= p.from && date <= p.to);
                      }).length;
                      const machineDays = allDays.map(date => {
                        const items = getItemsForDay(date).filter(r => Number(r.machineId) === m.id && r.done);
                        return { date, items };
                      }).filter(d => d.items.length > 0);
                      const totalTrips = machineDays.reduce((sum,d) => {
                        return sum + d.items.reduce((s,r) => {
                          const emp = data.users.find(u=>u.id===Number(r.assignedTo||r._creatorId));
                          return s + Number(data.workTime[emp?.id]?.[d.date]?.trips||0);
                        },0);
                      },0);
                      return (
                        <div key={m.id} style={S.card}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                            ...
                            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                              ...
                              {repairDays > 0 && <span style={S.tag({ background:"#fef3e2", color:"#f39c12" })}>🔧 {repairDays} дней ремонт</span>}
                            </div>
                          </div>
                          {machineDays.length===0 && <div style={{color:"#ccc",fontSize:13}}>Не использовалась в этом месяце</div>}
                          {machineDays.map(({date,items}) => items.map(r => {
                            const emp = data.users.find(u=>u.id===Number(r.assignedTo||r._creatorId));
                            const wt  = data.workTime[emp?.id]?.[date]||{};
                            return (
                              <div key={r.id} style={{ padding:"6px 0", borderTop:"1px solid #f5f5f5", fontSize:13 }}>
                                <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                                  ...
                                  <span style={{ color:"#333" }}>🚗 {emp?.name?.split(" ")[0]||"?"} {emp?.name?.split(" ")[1]||""}</span>
                                  ...
                                  <span style={S.tag({ background:"#e8f7ee", color:"#27ae60" })}>⏱ {wt.from||"—"}–{wt.to||"—"}</span>
                                </div>
                              </div>
                            );
                          }))}
                          {machineDays.length>0 && (
                            <div style={{ marginTop:8, paddingTop:8, borderTop:"2px solid #f0f0f0", display:"flex", gap:8, flexWrap:"wrap" }}>
                              <span style={S.tag({ background:"#e8f4fd", color:"#2d6a9f", fontWeight:700 })}>Итого рейсов: {totalTrips}</span>
                            </div>
                          )}
                        </div>
                      );
                ...
                {repSub==="drivers" && (
                  <div>
                    {drivers.map(drv => {
                      const workDays = allDays.map(date => {
                        const wt    = data.workTime[drv.id]?.[date] || {};
                        const items = getReqsForDateByEmployee(drv.id, date).filter(r => r.done);
                        if (items.length === 0) return null;
                        return { date, wt, items };
                      }).filter(Boolean);
                      ...
                      return (
                        <div key={drv.id} style={S.card}>
                          ...
                          {workDays.length===0 && <div style={{color:"#ccc",fontSize:13}}>Нет данных за этот месяц</div>}
                          {workDays.map(({date,wt,items}) => {
                            const machines = [...new Set(items.filter(r=>r.machineId).map(r=>data.machines.find(m=>m.id===Number(r.machineId))?.name).filter(Boolean))];
                            return (
                          ...
                          {workDays.length>0 && (
                            <div style={{ marginTop:8, paddingTop:8, borderTop:"2px solid #f0f0f0", display:"flex", gap:8, flexWrap:"wrap" }}>
                              <span style={S.tag({ background:"#e8f4fd", color:"#2d6a9f", fontWeight:700 })}>Итого: {workDays.length} дней · {totalHours.toFixed(1)} ч · {totalTrips} рейсов</span>
                            </div>
                          )}
                        </div>
                      );
            ...
            <div>
              {data.users.map(u => (
                <div key={u.id} style={{ ...S.card, display:"flex", alignItems:"center", gap:12, padding:"12px 14px" }}>
                  <span style={{ fontSize:22 }}>{u.role==="manager"?"👑":u.role==="driver"?"🚗":u.role==="supervisor"?"📋":"👤"}</span>
                  <div>
                    <div style={{ fontWeight:600, color:"#1e3a5f", fontSize:14 }}>{u.name}</div>
                    <div style={{ fontSize:12, color:"#888" }}>PIN: {u.pin} · {u.role==="manager"?"Руководитель":u.role==="driver"?"Водитель":u.role==="supervisor"?"Менеджер":"Сотрудник"}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            {currentUser.role === "driver" && (
              <div style={S.card}>
                ...
                {isPastDate(todayDate) && (
                  <div style={{ background:"#fdecea", borderRadius:8, padding:"8px 12px", marginBottom:10, fontSize:13, color:"#e74c3c" }}>
                    🔒 Редактирование прошедших дней недоступно
                  </div>
                )}
                <div style={{ marginBottom:10 }}>
                  <label style={{ fontSize:12, color:"#555", display:"block", marginBottom:4 }}>Название организации</label>
                  <input type="text" disabled={isPastDate(todayDate)}
                    value={data.workTime[currentUser.id]?.[todayDate]?.org || ""}
                    onChange={e => upd(d => {
                      if (!d.workTime[currentUser.id]) d.workTime[currentUser.id] = {};
                      if (!d.workTime[currentUser.id][todayDate]) d.workTime[currentUser.id][todayDate] = {};
                      d.workTime[currentUser.id][todayDate].org = e.target.value;
                    })}
                    placeholder="Название организации..."
                    style={{ ...S.input, width:"100%", opacity: isPastDate(todayDate) ? 0.5 : 1 }} />
                </div>
                <div style={{ marginBottom:10 }}>
                  <label style={{ fontSize:12, color:"#555", display:"block", marginBottom:4 }}>Маршрут</label>
                  <input type="text" disabled={isPastDate(todayDate)}
                    value={data.workTime[currentUser.id]?.[todayDate]?.route || ""}
                    onChange={e => upd(d => {
                      if (!d.workTime[currentUser.id]) d.workTime[currentUser.id] = {};
                      if (!d.workTime[currentUser.id][todayDate]) d.workTime[currentUser.id][todayDate] = {};
                      d.workTime[currentUser.id][todayDate].route = e.target.value;
                    })}
                    placeholder="Например: Уфа — Стерлитамак..."
                    style={{ ...S.input, width:"100%", opacity: isPastDate(todayDate) ? 0.5 : 1 }} />
                </div>
                <div>
                  <label style={{ fontSize:12, color:"#555", display:"block", marginBottom:4 }}>Количество рейсов</label>
                  <input type="number" min="0" disabled={isPastDate(todayDate)}
                    value={data.workTime[currentUser.id]?.[todayDate]?.trips || ""}
                    onChange={e => upd(d => {
                      if (!d.workTime[currentUser.id]) d.workTime[currentUser.id] = {};
                      if (!d.workTime[currentUser.id][todayDate]) d.workTime[currentUser.id][todayDate] = {};
                      d.workTime[currentUser.id][todayDate].trips = e.target.value;
                    })}
                    placeholder="Введите кол-во рейсов..."
                    style={{ ...S.input, width:"100%", opacity: isPastDate(todayDate) ? 0.5 : 1 }} />
                </div>
              </div>
            )}
            <div style={S.card}>
              ...
              <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
                <div>
                  <label style={{ fontSize:11, color:"#888", display:"block", marginBottom:2 }}>С</label>
                  <input type="time" value={todayWt.from||""} disabled={isPastDate(todayDate)} onChange={e => upd(d => {
                    if (!d.workTime[currentUser.id]) d.workTime[currentUser.id]={};
                    if (!d.workTime[currentUser.id][todayDate]) d.workTime[currentUser.id][todayDate]={};
                    d.workTime[currentUser.id][todayDate].from=e.target.value;
                  })} style={{ ...S.input, opacity: isPastDate(todayDate) ? 0.5 : 1 }} />
                </div>
                <div>
                  <label style={{ fontSize:11, color:"#888", display:"block", marginBottom:2 }}>До</label>
                  <input type="time" value={todayWt.to||""} disabled={isPastDate(todayDate)} onChange={e => upd(d => {
                    if (!d.workTime[currentUser.id]) d.workTime[currentUser.id]={};
                    if (!d.workTime[currentUser.id][todayDate]) d.workTime[currentUser.id][todayDate]={};
                    d.workTime[currentUser.id][todayDate].to=e.target.value;
                  })} style={{ ...S.input, opacity: isPastDate(todayDate) ? 0.5 : 1 }} />
                </div>
                ...
                <div key={r.id} style={S.card}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                    <button onClick={() => upd(d => {
                      ...
                      if (r._type==="task") { const t=d.tasks.find(x=>x.id===r.id); if(t) { t.done=!t.done; t.doneBy = t.done ? stamp : null; } }
                        ...
                        const uid = r._creatorId ?? currentUser.id;
                        const req = d.requests[uid]?.[todayDate]?.find(x=>x.id===r.id); if(req) { req.done=!req.done; req.doneBy = req.done ? stamp : null; }
                      }
                  ...
                  {!r._type && (
                    <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                      <div>
                        <label style={{ fontSize:11, color:"#aaa", display:"block", marginBottom:2 }}>Начало</label>
                        <input type="time" value={r.timeFrom||""} disabled={isPastDate(todayDate)} onChange={e => upd(d => {
                          const uid=r._creatorId??currentUser.id;
                          const req=d.requests[uid]?.[todayDate]?.find(x=>x.id===r.id); if(req) req.timeFrom=e.target.value;
                        })} style={{ ...S.input, fontSize:13, opacity:isPastDate(todayDate)?0.5:1 }} />
                      </div>
                      <div>
                        <label style={{ fontSize:11, color:"#aaa", display:"block", marginBottom:2 }}>Конец</label>
                        <input type="time" value={r.timeTo||""} disabled={isPastDate(todayDate)} onChange={e => upd(d => {
                          const uid=r._creatorId??currentUser.id;
                          const req=d.requests[uid]?.[todayDate]?.find(x=>x.id===r.id); if(req) req.timeTo=e.target.value;
                        })} style={{ ...S.input, fontSize:13, opacity:isPastDate(todayDate)?0.5:1 }} />
                      </div>
                      {r.timeFrom&&r.timeTo && (
                        <div style={{ background:"#e8f7ee", borderRadius:8, padding:"4px 10px", fontSize:13, color:"#27ae60", alignSelf:"flex-end" }}>
                          {calcHours(r.timeFrom,r.timeTo)} ч
                        </div>
                      )}
                    </div>
                  )}
                  <div style={{ marginTop:8 }}>
                    <label style={{ fontSize:11, color:"#aaa", display:"block", marginBottom:2 }}>💬 Комментарий (поломка, простой, причина задержки...)</label>
                    <input type="text" value={r.comment||""} disabled={isPastDate(todayDate)}
                      onChange={e => upd(d => {
                        if (r._type==="task") {
                          const t=d.tasks.find(x=>x.id===r.id); if(t) t.comment=e.target.value;
                        } else {
                          const uid=r._creatorId??currentUser.id;
                          const req=d.requests[uid]?.[todayDate]?.find(x=>x.id===r.id); if(req) req.comment=e.target.value;
                        }
                      })}
          ...
          <div>
            ...
            {myTasks.map(task => (
              <div key={task.id} style={{ ...S.card, display:"flex", alignItems:"center", gap:10 }}>
                ...
                <span style={S.tag(task.done?{ background:"#e8f7ee", color:"#27ae60" }:{ background:"#fff3cd", color:"#e67e22" })}>
